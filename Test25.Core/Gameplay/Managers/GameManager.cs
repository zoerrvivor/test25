...
using Test25.Core.Gameplay.Entities.Projectiles;
...
namespace Test25.Core.Gameplay.Managers
{
...

    {
        ...
        public void SpawnEntity(string type, Vector2 position, Color? color = null, bool isAi = true)
        {
            if (type == "Tank")
            {
                int id = Players.Count;
                string name = $"Tank {id + 1}";
                Color c = color ?? (id == 0 ? Color.Red : Color.Blue);
                AddPlayer(new Tank(id, name, position, c, _tankBodyTexture, _tankBarrelTexture, isAi, null));
            }
            else if (type == "Crate" || type == "Tree" || type == "Building")
            {
                // For now, treat as non-tank entities if we have a way to represent them.
                // Building ruins are in DecorationManager.
                // Maybe we can add a way to manually place decorations.
                DecorationManager.AddDecoration(type, position, Terrain);
            }
        }
        ...
        public void HandleTankDeath(Tank tank, Tank? killer = null)
        {
            ...
            SoundManager.PlaySound("heavy_explosion");
        ...
        private void DrawTooltip(SpriteBatch spriteBatch, SpriteFont font)
        {
            ...
            if (_hoveredTank != null && _hoveredTank.IsAi && _hoveredTank.Personality != null)
            {
