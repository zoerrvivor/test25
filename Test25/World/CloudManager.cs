...
namespace Test25.World
{
    ...
    public class CloudManager
    {
        ...
        public CloudManager(Texture2D cloudTexture, int screenWidth, int screenHeight, int cloudCount = 5)
        {
            ...
            {
                var startPos = new Vector2(_rand.Next(0, _screenWidth), _rand.Next(0, _screenHeight / Constants.CloudSpawnHeightDivisor));
                float speed = (float)_rand.NextDouble() * (Constants.CloudMaxSpeed - Constants.CloudMinSpeed) + Constants.CloudMinSpeed;
                float scale = (float)_rand.NextDouble() * (Constants.CloudMaxScale - Constants.CloudMinScale) + Constants.CloudMinScale;
